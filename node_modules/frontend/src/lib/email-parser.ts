...
/**
 * Generate inputs for the BRACU email verifier circuit using preprocessed headers
 * This function accepts already-processed header bytes to work around circuit size constraints
 */
export async function generateEmailVerifierInputsWithPreprocessedHeader(
  processedHeaderBytes: Uint8Array,
  rawEmail: string
): Promise<EmailVerifierInputs> {
  console.log('📧 Generating circuit inputs with preprocessed header...')
  console.log(`   Preprocessed header size: ${processedHeaderBytes.length} bytes`)

  // Extract From address from raw email (we need this for validation)
  const { fromAddress, fromDomain } = extractFromAddress(rawEmail)
  
  // Find From header indices in the processed header
  const headerString = new TextDecoder().decode(processedHeaderBytes)
  const { headerIndex, headerLength, addressIndex, addressLength } = findFromIndices(headerString, fromAddress)

  console.log(`   Found From header at index ${headerIndex}, length ${headerLength}`)
  console.log(`   Found From address at index ${addressIndex}, length ${addressLength}`)

  // CRITICAL: Ensure indices are within bounds to prevent circuit overflow
  const calculatedHeaderLength = processedHeaderBytes.length
  const safeHeaderIndex = Math.min(headerIndex, calculatedHeaderLength - headerLength)
  const safeAddressIndex = Math.min(addressIndex, calculatedHeaderLength - addressLength)
  ...
  // For preprocessed headers, we need to extract DKIM info from the original email
  // but use the processed header for circuit inputs
  const dkimInfo = extractDKIMInfo(rawEmail)

  return {
    emailHeader: Array.from(processedHeaderBytes),
    emailHeaderLength: calculatedHeaderLength,
    pubkey: dkimInfo.pubkeyModulus,
    pubkeyRedc: dkimInfo.pubkeyRedc,
    signature: dkimInfo.signature,
    fromHeaderIndex: safeHeaderIndex,
    fromHeaderLength: headerLength,
    fromAddressIndex: finalAddressIndex,
    fromAddressLength: addressLength,
...
/**
 * Extract DKIM information from raw email (helper for preprocessed header function)
 */
function extractDKIMInfo(rawEmail: string): {
  pubkeyModulus: string[]
  pubkeyRedc: string[]
  signature: string[]
} {
  // This is a simplified version - in production, you'd use the full DKIM parsing
  // For now, we'll use a fallback approach
  
  // Look for DKIM signature in the raw email
  const dkimMatch = rawEmail.match(/^DKIM-Signature:.*?\r?\n(?!\s)/ims)
  if (!dkimMatch) {
    throw new Error('No DKIM signature found in email')
  }

  // For now, return dummy values - in production, you'd parse the actual DKIM signature
  // and extract the RSA public key and signature
  console.warn('⚠️  Using fallback DKIM extraction - implement proper parsing for production')
  
  return {
    pubkeyModulus: Array(18).fill('0x1234567890abcdef'), // Dummy values
    pubkeyRedc: Array(18).fill('0x0'), // Dummy values
    signature: Array(18).fill('0x1234567890abcdef') // Dummy values
  }
}
