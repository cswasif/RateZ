...
exports.hashRSAPublicKey = hashRSAPublicKey;
 ...
 */
async function hashRSAPublicKey(modulusLimbs, redcLimbs) {
    ...
    if (modulusLimbs.length !== NUM_TOTAL_LIMBS ||
        redcLimbs.length !== NUM_TOTAL_LIMBS) {
        throw new Error(`Expected ${NUM_TOTAL_LIMBS} limbs for both modulus and redc, but received ${modulusLimbs.length} and ${redcLimbs.length}`);
    }
    const poseidon = await (0, circomlibjs_1.buildPoseidon)();
    const chunk1_composed = [];
    const chunk2_composed = [];
    for (let i = 0; i < NUM_ELEMENTS_PER_POSEIDON_CHUNK; i++) {
        ...
        chunk2_composed.push(redc_limb_A * SHIFT_VALUE_FOR_COMPOSITION + redc_limb_B);
    }
